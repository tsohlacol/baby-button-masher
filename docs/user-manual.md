# Baby Button Masher - User Manual & Operating Guide

Welcome to **Baby Button Masher (BBM)**. This manual provides clear instructions on how to install, configure, operate, and exit the sensory toddler lockdown portal safely.
...
### Fresh Installation
1. Download the latest installer package `BBM_Setup_v1.0.exe` from the Github Releases drawer.
...
4. Select your installation path (defaults to `%PROGRAMFILES%\BabyButtonMasher`) and specify whether you want a Desktop Shortcut icon generated.
5. Click **Install**.
6. (Optional) Run of boot: Check the box "Launch Baby Button Masher automatically on computer boot" to make it lock the laptop standard displays as soon as your Windows user profile initializes.

---

## 2. Dynamic Configurations & Game Room Selection

Once launched, Baby Button Masher locks the computer screen into an interactive gaming shield. While in sandbox play, the default windows keyboard bindings (Windows Keys, Alt+Tab, etc.) are swallowed safely.
...
4. **Deleting Associations**: Clicking the **Trash Can** icon next to any active configuration instantly deletes the overridden word association, seamlessly reverting BBM to its default high-quality learning dictionary items.

---

## 3. How to Exit (The Parent Override Hatch)

Because children will repeatedly press standard letters (`E`, `X`, `I`, `T`) or escape keys, the parent exit mechanism requires cognitive or code-based verification. BBM supports multiple exit options:
...
2. Because the Secure Attention Sequence (SAS) is managed directly by the Windows Kernel (`winlogon.exe`), **it will bypass Baby Button Masher completely**.
3. Select **Task Manager** from the overlay screen.
4. Locate `BabyButtonMasher.exe` (or `node` during local developer modes) in the active Processes list.
...
* **To Uninstall**: 
  ...
  2. Locate **Baby Button Masher** inside the programs library.
...
To guarantee the child lockdown portal is resilient against input crashes, memory leaks, and typical security escalation paths, BBM maintains a comprehensive unit testing framework and automated five-tier security pipeline.
